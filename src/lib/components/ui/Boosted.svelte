<script lang="ts">
	...
	$: label = `${boosted ? `Boosted ${toProperCase(kind)}: ${boosted.boostname ?? ''}` : `No boosted ${kind} selected`}${
		stale ? ' (update unavailable)' : ''
	}`;
</script>
...
<div
	...
	<div
		...
		<span class="rounded-full bg-warning-500 ring-2 ring-primary-600/50 p-0.5">
			<Fa
