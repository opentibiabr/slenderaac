<script lang="ts">
	...
	function attributes(attrs: Record<string, string>, url: URL) {
...
<!-- Keep inline text spacing from the captured document. -->
<!-- prettier-ignore -->
{#each nodes as node}{#if typeof node === 'string'}{node}{:else}{#if allowedTags.has(elementNode(node).tag)}{#if elementNode(node).tag === 'img' && imageLink(elementNode(node).imageHref)}<a href={imageLink(elementNode(node).imageHref)} target="_blank" rel="noreferrer" aria-label="Open full-size image"><img {...attributes(elementNode(node).attrs, $page.url)} alt={elementNode(node).attrs.alt ?? ''} /></a>{:else if elementNode(node).tag === 'img' || elementNode(node).tag === 'br'}<svelte:element this={elementNode(node).tag} {...attributes(elementNode(node).attrs, $page.url)} />{:else}<svelte:element this={elementNode(node).tag} {...attributes(elementNode(node).attrs, $page.url)}><svelte:self nodes={elementNode(node).children} /></svelte:element>{/if}{/if}{/if}{/each}
