...
<div class="table-container">
	<table class="table table-hover table-auto">
		<thead>
			<tr class="[&>th]:!p-2">
				{#if ranked}<th class="w-12">Rank</th>{/if}
				...
				<TableHeader {sort} {order} col="level" class="w-16"
					>{$_('level')}</TableHeader>
				{#if skill}<TableHeader {sort} {order} col={skill} class="w-24"
		...
		<tbody class="transition-all duration-300 ease-in-out">
			{#each characters as character (character.id)}
				<tr
					...
					transition:fly|local={{ duration: 300, y: -20, easing: cubicInOut }}>
					{#if ranked}<td>{'rank' in character && character.rank}</td>{/if}
					<td>
						<a
					...
					<td class=" w-fit relative">
						<div class="flex flex-col w-fit justify-center">
							<span class="font-semibold flex flex-row gap-1 items-center">
								...
								{#if character.isMain}<MainCharacterIndicator />{/if}
							</span>
					...
					{#if skill}<td>{'skill' in character && character.skill}</td>{/if}
				</tr>
			{/each}
		</tbody>
	</table>
